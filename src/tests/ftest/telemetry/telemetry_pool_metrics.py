...
class TelemetryPoolMetrics(IorTestBase, TestWithTelemetry):
    ...
    def get_expected_values(self):
        ...
        pool_ops_minmax = {
            "SX": {
                ...
                "engine_pool_resent": None,
                "engine_pool_restarted": None,
                "engine_pool_retry": None
            },
            "RP_3GX": {
                "engine_pool_ops_fetch": (
                    ops_number + 7,
                ...
                "engine_pool_xferred_update": (
                    ...
                    (3 * ops_number + 1) * self.ior_transfer_size
                ),
                "engine_pool_resent": None,
                "engine_pool_restarted": None,
                "engine_pool_retry": None
    ...
    def test_telemetry_pool_metrics(self):
        ...
        for name in metric_names:
            ...
            if expected_values[name] is None:
                self.log.debug(
                    "FIXME: Metric {}={} (oclass={})".format(name, val, self.dfs_oclass))
                continue
