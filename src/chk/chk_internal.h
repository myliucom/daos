 ...
 *
 *   Handle dangling pool requires all check engines to report their known pools (shards), then
 ...
 *
 *   2. Explicitly specify 'dangling' flag when check start, in spite of it is for all pools or
 *      just against the specified pool list.
 *
 *   NOTE: Similar as 'reset' flag, the 'dangling' flag is also not stored persistently, instead,
...
int chk_ranks_prepare(struct chk_instance *ins, uint32_t rank_nr, d_rank_t *ranks,
